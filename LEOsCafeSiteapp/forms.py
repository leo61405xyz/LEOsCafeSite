...
class CafeForm(forms.Form):
    boardcbname = forms.CharField(label="Coffee Bean Name", initial='', max_length=20, required=True, widget=forms.TextInput(attrs={'class': 'orderform-input-text-area'}))
    boardcbcoffeeshop = forms.CharField(label="Coffee Shop", initial='', max_length=20, required=True, widget=forms.TextInput(attrs={'class': 'orderform-input-text-area'}))
    boardcbplace = forms.CharField(label="Coffee Bean Place", initial='', max_length=20, required=True, widget=forms.TextInput(attrs={'class': 'orderform-input-text-area'}))
    boardcbroasting = forms.CharField(label="Coffee Bean Roasting", initial='', max_length=20, required=True, widget=forms.TextInput(attrs={'class': 'orderform-input-text-area'}))
    boardcbprice = forms.CharField(label="Coffee Bean Price", initial='', max_length=20, required=True, widget=forms.TextInput(attrs={'class': 'orderform-input-text-area'}))
